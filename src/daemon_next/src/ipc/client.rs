...
impl IpcClient {
    #[cfg(unix)]
    pub async fn call(socket_path: impl AsRef<std::path::Path>, method: &str, params: Option<Value>) -> Result<JsonRpcResponse> {
    ...
    #[cfg(windows)]
    pub async fn call(pipe_name: &str, method: &str, params: Option<Value>) -> Result<JsonRpcResponse> {
    ...
    async fn call_on_stream<S>(stream: S, method: &str, params: Option<Value>) -> Result<JsonRpcResponse>
    where
        S: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin,
    {
        ...
        writer.write_all(text.as_bytes()).await.context("failed to write request")?;
