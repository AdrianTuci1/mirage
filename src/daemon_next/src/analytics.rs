...
cfg_if::cfg_if! {
    if #[cfg(feature = "duckdb")] {
        ...
        ///
        /// The database lives inside `data_dir/analytics.duckdb`. When the `duckdb`
        /// feature is enabled, the DuckDB runtime is compiled/linked in. The module
        /// manager can also download an external shared library, but the bundled
        /// build is used by default.
        ...
        /// Stub analytics engine used when the `duckdb` feature is disabled.
        /// All operations return an error explaining that the module is not installed.
        pub struct Analytics {
            _db_path: PathBuf,
        }
        ...
        impl Analytics {
            pub fn open(config: &DaemonConfig) -> Result<Self> {
                Ok(Self {
                    _db_path: config.data_dir.join("analytics.duckdb"),
                })
            }

            pub fn execute(&self, _sql: &str) -> Result<usize> {
                Err(anyhow!("DuckDB analytics module is not installed. Download it via the Modules settings."))
            }

            pub fn query(&self, _sql: &str) -> Result<Vec<Map<String, Value>>> {
                Err(anyhow!("DuckDB analytics module is not installed. Download it via the Modules settings."))
            }

            pub fn ingest_csv(&self, _path: impl AsRef<std::path::Path>, _table_name: &str) -> Result<usize> {
                Err(anyhow!("DuckDB analytics module is not installed. Download it via the Modules settings."))
            }

            pub fn ingest_parquet(&self, _path: impl AsRef<std::path::Path>, _table_name: &str) -> Result<usize> {
                Err(anyhow!("DuckDB analytics module is not installed. Download it via the Modules settings."))
            }
